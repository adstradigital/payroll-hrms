...
import {
    ...
    CheckCircle2, AlertCircle, Loader2, Send, X, ClipboardList
} from 'lucide-react';
import { getEmployeeById, getMyProfile, getEmployeeFieldDefinitions } from '@/api/api_clientadmin';
...
import ProfileLeave from './Leave/ProfileLeave';
import ProfileWorkShift from './WorkShift/ProfileWorkShift';
import ProfileAllowances from './Allowances/ProfileAllowances';
import ProfilePenalty from './Penalty/ProfilePenalty';
import ProfileAssets from './Assets/ProfileAssets';
import ProfilePerformance from './Performance/ProfilePerformance';
import ProfileBonusPoints from './Bonus/ProfileBonusPoints';
import ProfileInterview from './Interview/ProfileInterview';
import ProfileResignation from './Resignation/ProfileResignation';
...
export default function EmployeeProfile({ employeeId, onBack }) {
    ...
    const fetchProfile = async () => {
        ...
        try {
            ...
            try {
                const fieldsRes = await getEmployeeFieldDefinitions();
                setCustomFieldsDef((fieldsRes.data.results || fieldsRes.data || []).filter(v => v.is_active));
            } catch (err) { console.error('Fetch Fields Err', err); }
            setLoading(false);
        } catch (err) {
    ...
    return (
        <div className="emp-profile-view-v2">
            {/* --- Header --- */}
            ...
            <div className="content-grid">
                ...
                <main className="main-content">
                    ...
                    {activeTab === 'leave' && <ProfileLeave employeeId={employeeId || employee.id} />}

                    {activeTab === 'work_type' && <ProfileWorkShift employeeId={employeeId || employee.id} employee={employee} />}

                    {activeTab === 'allowance' && <ProfileAllowances employeeId={employeeId || employee.id} />}

                    {activeTab === 'penalty' && <ProfilePenalty employeeId={employeeId || employee.id} />}

                    {activeTab === 'assets' && <ProfileAssets employeeId={employeeId || employee.id} />}

                    {activeTab === 'bonus' && <ProfileBonusPoints employeeId={employeeId || employee.id} />}
                    {activeTab === 'performance' && <ProfilePerformance employeeId={employeeId || employee.id} />}
                    {activeTab === 'interview' && (
                        <ProfileInterview
                            employeeId={employeeId || employee.id}
                            employeeUserId={employee.user?.id || employee.user || employee.user_id || null}
                        />
                    )}
                    {activeTab === 'resignation' && <ProfileResignation employee={employee} />}
                    {activeTab === 'onboarding' && <ProfileOnboarding employeeId={employeeId || employee.id} />}
