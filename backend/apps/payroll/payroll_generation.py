...
def _generate_summary_report(company_id, month, year):
    ...
    try:
        ...
        return Response({'error': f"Summary Report Error: {str(e)}"}, status=500)
...
def _generate_detailed_report(company_id, month, year):
    ...
    try:
        ...
        return Response({'error': f"Detailed Report Error: {str(e)}"}, status=500)

def _generate_statutory_report(company_id, month, year):
...
def get_payroll_reports(request):
    ...
    try:
        ...
        if report_type == 'summary':
            ...
            return _generate_statutory_report(company_id, int(month), int(year))
        else:
            return Response(
